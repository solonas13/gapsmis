...
def run_needle ( errors, gap_open_pen, gap_ext_pen ):
  os . system ( "needle tmp_a.seq tmp_b.seq -datafile EDNAFULL -gapopen " + str ( gap_open_pen ) + " -gapextend " + str ( gap_ext_pen ) + " -outfile tmp.needle 2> /dev/null" );
  return ( process_needle ( "tmp.needle", errors ) )

def process_needle ( filename, errors):
  f = open ( filename, 'r' )
  s = f . read ()
  f . close ()
...
def main ( argv = None ):
  ...
  if len ( argv ) != 4:
    print " usage: ./needle_bench.py <gap open penalty [float]> <gap extend penalty [float]> <num of tests [int]>";
    sys . exit ( 0 )

  gap_open_pen = float ( argv[1] )
  gap_ext_pen  = float ( argv[2] )

  match_needle_correct = 0
  match_needle_valid  = 0
  ...
  while 1:
    ...
    needle = run_needle ( errors, gap_open_pen, gap_ext_pen )
    if needle == 1: match_needle_correct += 1; match_needle_valid += 1;
    elif needle == 2: 
      match_needle_valid  += 1;
      debug ( 'needle.inc', query, target)
    else:
      debug ( 'needle.inv', query, target)
  ...
  print "needle"
  print "   Valid: " +  str ( match_needle_valid ) + " (Invalid alignments: needle.inv)"
  print "   Correct:    " +  str ( match_needle_correct ) + " (Valid but not Correct: needle.inc)"

  os . system ( "rm tmp_a.seq" )
  os . system ( "rm tmp_b.seq" )
  os . system ( "rm tmp.needle" )
